...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS','').split()] #Database Channels
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS','').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else ADMINS
AUTH_CHANNEL = environ.get('AUTH_CHANNEL','') #ForceSubcribe Channel
AUTH_GROUPS = [int(ch) for ch in environ.get("AUTH_GROUPS",'').split()] #Allowed Chat IDs
